...
from core.errors import RenderAgentError
...
def test_render_agent_requires_media_for_non_script_jobs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setenv("OUTPUT_DIR", str(tmp_path / "outputs"))
    get_settings.cache_clear()
    project_dir = tmp_path / "outputs" / "projects" / "rd2"
    project_dir.mkdir(parents=True)
    project = ProjectMetadata(
        project_id="rd2", source_type=SourceType.UPLOAD, raw_text="hello"
    )

    with pytest.raises(RenderAgentError, match="no media source available"):
        RenderAgent().run(
            project,
            project_dir=project_dir,
            config=VideoJobConfig(platform="TikTok", reframe_aspect="9:16"),
