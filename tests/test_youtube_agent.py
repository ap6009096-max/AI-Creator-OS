...
def test_youtube_agent_writes_source_metadata(
    ...
    assert result.metadata.source_status == YouTubeSourceStatus.READY_FOR_PIPELINE
    ...
    assert data["source_status"] == "ready_for_pipeline"
...
def test_youtube_agent_downloads_authorized_media(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("OUTPUT_DIR", str(tmp_path / "outputs"))
    ...
    client = _mock_client({"title": "Downloaded", "author_name": "Channel"})
    ...
    class _FakeDownloader:
        ...
        def download(self, _urls):
            ...
            output.write_bytes(b"authorized media")

    monkeypatch.setattr(
        "tools.youtube.oembed_provider.YoutubeDL",
        _FakeDownloader,
    )
    provider = OEmbedYouTubeProvider(client=client)
    result = YouTubeAgent(provider=provider).run(
        ProjectMetadata(
            project_id="yt-download",
            source_type=SourceType.YOUTUBE,
            youtube_url=WATCH_URL,
        ),
    ...
    assert result.local_media_path is not None
    assert Path(result.local_media_path).is_file()
    assert Path(result.local_media_path).name == "media.mp4"
    state = result.to_state_dict()
    assert state["source_metadata"]["local_media_path"] == result.local_media_path
    get_settings.cache_clear()
