"""Export caption cues to SRT, VTT, and ASS."""
...
def _karaoke_text(cue: CaptionCue, style: CaptionStylePreset) -> str:
    if style.animation != "word_highlight" or not cue.words:
        return _wrap_lines(
            _display_text(cue),
    ...
    for w in cue.words:
        ...
        if w.highlighted or token in cue.highlighted_tokens:
            parts.append(f"{{\\k{dur_cs}\\c{style.ass_style.highlight_color}}}{token}")
        else:
...
def render_ass(
    ...
    """Render ASS with platform-safe margins; karaoke when animation needs it."""
    header = [
        "[Script Info]",
        "Title: AI Creator OS Captions",
        "ScriptType: v4.00+",
        "WrapStyle: 0",
    ...
    for cue in track.cues:
        text = _karaoke_text(cue, style)
        # Pop animation: brief scale bump via ASS override tags
        if style.animation == "pop" and "\\k" not in text:
            text = "{\\fscx110\\fscy110}" + text
...
def ass_is_useful(style: CaptionStylePreset, *, burn_in: bool) -> bool:
    ...
    if burn_in:
        return True
    return style.animation in ("word_highlight", "pop")
