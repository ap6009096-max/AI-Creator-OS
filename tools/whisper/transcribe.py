...
def load_whisper_model(model_name: str):
    ...
    try:
        ...
        message = str(exc)
        if "WinError 4551" in message or "Application Control policy" in message:
            raise TranscriptAgentError(
                "Whisper cannot load Torch because Windows Application Control "
                "blocked a Torch DLL. Move/recreate the .venv outside OneDrive "
                "or ask your administrator to allow the official Torch package, "
                "then reinstall torch and torchaudio."
            ) from exc
        raise TranscriptAgentError(f"Whisper runtime could not load Torch: {exc}") from exc
