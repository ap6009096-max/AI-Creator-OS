...
Local media binaries are downloaded only when YOUTUBE_DOWNLOAD_ENABLED=true.
...
class OEmbedYouTubeProvider:
    ...
    def prepare_source(
        ...
        if settings.youtube_download_enabled:
            local_media_path = self._download_media(metadata, source_dir)
            prepared_meta = prepared_meta.model_copy(
                update={
                    "notes": (
                        f"{metadata.notes} Downloaded authorized local media to "
                        f"{Path(local_media_path).name}."
                    ).strip(),
                }
            )
        else:
            prepared_meta = prepared_meta.model_copy(
                update={
                    "notes": (
                        f"{metadata.notes} Status ready_for_pipeline with "
                        "awaiting_authorized_media (download disabled)."
                    ).strip(),
                }
            )
        # Reflect dual status intent in notes; primary status for pipeline handoff
    ...
    def _download_media(
        ...
        """Download one authorized video file for local analysis/transcription."""
        output_template = str(source_dir / "media.%(ext)s")
        options = {
            "format": "bv*+ba/b",
        ...
        try:
            with YoutubeDL(options) as downloader:
                downloader.download([metadata.canonical_url or metadata.url])
        except Exception as exc:  # noqa: BLE001
            raise YouTubeAgentError(
                "YouTube media download failed. Verify the URL, authorization, "
                f"and FFmpeg setup: {exc}"
            ) from exc

        candidates = sorted(source_dir.glob("media.*"))
        media = next(
        ...
        if media is None or not media.is_file() or media.stat().st_size <= 0:
            raise YouTubeAgentError(
                "YouTube media download completed without a usable local media file."
            )
