...
class InputAgent(BaseAgent):
    ...
    def run(self, request: VideoJobRequest | dict[str, Any], **_: Any) -> InputAgentResult:
        ...
        metadata = ProjectMetadata(
            ...
            source_path=(
                ...
                else job.local_media_path
                if source_type == SourceType.YOUTUBE
                else ""
            ),
            youtube_url=job.youtube_url if source_type == SourceType.YOUTUBE else "",
            raw_text=job.script_text if source_type == SourceType.SCRIPT else "",
    ...
    def _validate(self, job: VideoJobRequest, source_type: SourceType) -> None:
        if source_type == SourceType.YOUTUBE:
            ...
            if job.local_media_path:
                self._validate_local_media(job.local_media_path)
    ...
    def _validate_local_media(self, media_path: str) -> None:
        path = Path(media_path)
        if not path.is_file():
            raise InputValidationError(f"Authorized local media file not found: {path}")
        if path.suffix.lower() not in ALLOWED_UPLOAD_EXTENSIONS:
            raise InputValidationError(
                f"Unsupported local media format '{path.suffix.lower()}'. "
                f"Allowed: {', '.join(sorted(ALLOWED_UPLOAD_EXTENSIONS))}"
            )
