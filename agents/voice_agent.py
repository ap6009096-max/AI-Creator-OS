"""Voice Agent — resolve voice plan with TTS provider abstraction (no Gemini)."""
...
from schemas.av_plan import VoicePack, VoiceResult
...
class VoiceAgent(BaseAgent):
    """Write analysis/voice_plan.json; preserve original when TTS unavailable."""
    ...
    def run(
        ...
        **_: Any,
        ...
        messages = [
            ...
            f"[{self.name}] Wrote analysis/voice_plan.json",
        ]
        logger.info(
            "VoiceAgent ready project_id=%s voice=%s preserve=%s",
            project_id,
            pack.preset.name,
